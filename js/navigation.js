...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/navigation/information/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].number == "" || dataJson[i].number == null || dataJson[i].number == undefined) {
                            dataJson[i].number = "无";
                        } else {
                            dataJson[i].number = dataJson[i].number;
                        }
                        if (dataJson[i].lampQuality == "" || dataJson[i].lampQuality == null || dataJson[i].lampQuality == undefined) {
                            dataJson[i].lampQuality = "无";
                        } else {
                            dataJson[i].lampQuality = dataJson[i].lampQuality;
                        }
                        if (String(dataJson[i].lampHeight) == "" || dataJson[i].lampHeight == null || dataJson[i].lampHeight == undefined) {
                            dataJson[i].lampHeight = "无";
                        } else {
                            dataJson[i].lampHeight = dataJson[i].lampHeight;
                        }
                        if (String(dataJson[i].range) == "" || dataJson[i].range == null || dataJson[i].range == undefined) {
                            dataJson[i].range = "无";
                        } else {
                            dataJson[i].range = dataJson[i].range;
                        }
                        if (dataJson[i].structure == "" || dataJson[i].structure == null || dataJson[i].structure == undefined) {
                            dataJson[i].structure = "无";
                        } else {
                            dataJson[i].structure = dataJson[i].structure;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].lampQuality + "</td>\n" +
                            "            <td>" + dataJson[i].lampHeight + "</td>\n" +
                            "            <td>" + dataJson[i].range + "</td>\n" +
                            "            <td>" + dataJson[i].structure + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    navigationIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                navigationIds.splice(0,navigationIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var navigationId=$(this).siblings("td").eq(0).text();
                    navigationIds.push(navigationId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(navigationIds.indexOf(txt) == "-1"){
    
                    }else{
                        navigationIds.splice(navigationIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#navigation-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#navigation-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#navigation-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".navigation-tips").hide();
        $("#add-pop .imgList-navigation").hide();
        $("#edit-pop .imgList-navigation").hide();
        $("#add-pop .imgList-navigation").html("");
        $("#edit-pop .imgList-navigation").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".navigation-tips").show();
            $(".navigation-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".navigation-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".navigation-tips").show();
            $(".navigation-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".navigation-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-navigation").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                    ...
                    $("#add-pop .imgList-navigation").append(imgList);
                    uploadImgDelAdd();
    ...
    function uploadImgDelAdd(){
        $(".seawallNumber-imgItem").hover(function(){
            var href=$(this).find("span.url").text();
        ...
        $(".upload-delIcon").click(function(){
            $(this).parent().remove();
            var url="/image"+($(this).siblings("span.url").text()).split("image")[1];
    ...
    $("#addSure").click(function () {
        ...
        var lampQuality=$("#quality").val();
        var lampHeight=$("#height").val();
        var range=$("#range").val();
        ...
        console.log(lampHeight);
        console.log(range);
        if($("#add-pop").find("div.imgList-navigation").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/navigation/information/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".navigation-tips").show();
            $(".navigation-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".navigation-tips").hide();
            },1000);
        }else if(number==""){
            $(".navigation-tips").show();
            $(".navigation-tips").find("span").text("编号不能为空");
            setTimeout(function () {
                $(".navigation-tips").hide();
            },1000);
        }else if(lampHeight!=""){
            if(!(/^(-?\d+)(\.\d+)?$/.test(lampHeight))){
                $(".navigation-tips").show();
                $(".navigation-tips").find("span").text("灯高必须为数字类型");
                setTimeout(function () {
                    $(".navigation-tips").hide();
                },1000);
            }else{
                ...
                if(range!=""){
                    if(!(/^(-?\d+)(\.\d+)?$/.test(range))){
                        $(".navigation-tips").show();
                        $(".navigation-tips").find("span").text("射程必须为数字类型");
                        setTimeout(function () {
                            $(".navigation-tips").hide();
                        },1000);
                    }else{
    ...
    //批量删除
    $("#navigation-delAll").click(function () {
        $("#delAll-pop").show();
        if(navigationIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#navigation-delAll").removeClass("table-menuClick");
        $("#navigation-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/navigation/information/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/navigation/information/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.number==""||dataJson.number==null||dataJson.number==undefined){
                        dataJson.number="无";
                    }else{
                        dataJson.number=dataJson.number;
                    }
                    if(dataJson.lampQuality==""||dataJson.lampQuality==null||dataJson.lampQuality==undefined){
                        dataJson.lampQuality="无";
                    }else{
                        dataJson.lampQuality=dataJson.lampQuality;
                    }
                    if(String(dataJson.lampHeight)==""||dataJson.lampHeight==null||dataJson.lampHeight==undefined){
                        dataJson.lampHeight="无";
                    }else{
                        dataJson.lampHeight=dataJson.lampHeight;
                    }
                    if(String(dataJson.range)==""||dataJson.range==null||dataJson.range==undefined){
                        dataJson.range="无";
                    }else{
                        dataJson.range=dataJson.range;
                    }
                    if(dataJson.structure==""||dataJson.structure==null||dataJson.structure==undefined){
                        dataJson.structure="无";
                    }else{
                        dataJson.structure=dataJson.structure;
                    }
                    ...
                    $("#numberInfo").text(dataJson.number);
                    $("#qualityInfo").text(dataJson.lampQuality);
                    $("#heightInfo").text(dataJson.lampHeight);
                    $("#rangeInfo").text(dataJson.range);
                    $("#structureInfo").text(dataJson.structure);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-navigation").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-navigation").append(imgList);
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            navigationId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/navigation/information/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.number==""||dataJson.number==null||dataJson.number==undefined){
                        dataJson.numbernumber="";
                    }else{
                        dataJson.number=dataJson.number;
                    }
                    if(dataJson.lampQuality==""||dataJson.lampQuality==null||dataJson.lampQuality==undefined){
                        dataJson.lampQuality="";
                    }else{
                        dataJson.lampQuality=dataJson.lampQuality;
                    }
                    if(String(dataJson.lampHeight)==""||dataJson.lampHeight==null||dataJson.lampHeight==undefined){
                    ...
                    $("#numberEdit").val(dataJson.number);
                    $("#qualityEdit").val(dataJson.lampQuality);
                    $("#heightEdit").val(dataJson.lampHeight);
                    $("#rangeEdit").val(dataJson.range);
                    $("#structureEdit").val(dataJson.structure);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-navigation").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var lampQuality=$("#qualityEdit").val();
                var lampHeight=$("#heightEdit").val();
                var range=$("#rangeEdit").val();
                var structure=$("#structureEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-navigation").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#navigation-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/navigation/information/export?name="+name;
            $.ajax({
                url: srcPath + "admin/navigation/information/export?name="+name,
                type: "get",
