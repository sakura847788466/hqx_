...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/dangerous/chemicals/facility/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].dangerousType == "" || dataJson[i].dangerousType == null || dataJson[i].dangerousType == undefined) {
                            dataJson[i].dangerousType = "无";
                        } else {
                            dataJson[i].dangerousType = dataJson[i].dangerousType;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].dangerousType + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    chemicalIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                chemicalIds.splice(0,chemicalIds.length);
            }
        });
        $(".table").find("tbody tr").click(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var chemicalId=$(this).siblings("td").eq(0).text();
                    chemicalIds.push(chemicalId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(chemicalIds.indexOf(txt) == "-1"){

                    }else{
                        chemicalIds.splice(chemicalIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#chemical-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#chemical-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#chemical-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".chemical-tips").hide();
        $("#add-pop .imgList-chemical").hide();
        $("#add-pop .imgList-chemical").html("");
        $("#edit-pop .imgList-chemical").hide();
        $("#edit-pop .imgList-chemical").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".chemical-tips").show();
            $(".chemical-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".chemical-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".chemical-tips").show();
            $(".chemical-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".chemical-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-chemical").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        var name=$("#name").val();
        var type=$("#type").val();
        var dangerousType=$("#dangerous").val();
        var area=$("#area").val();
        ...
        if($("#add-pop").find("div.imgList-chemical").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/dangerous/chemicals/facility/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".chemical-tips").show();
            $(".chemical-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".chemical-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#chemical-delAll").click(function () {
        $("#delAll-pop").show();
        if(chemicalIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#chemical-delAll").removeClass("table-menuClick");
        $("#chemical-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/dangerous/chemicals/facility/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/dangerous/chemicals/facility/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.dangerousType==""||dataJson.dangerousType==null||dataJson.dangerousType==undefined){
                        dataJson.dangerousType="无";
                    }else{
                        dataJson.dangerousType=dataJson.dangerousType;
                    }
                    ...
                    $("#dangerousInfo").text(dataJson.dangerousType);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-chemical").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-chemical").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            chemicalId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/dangerous/chemicals/facility/detail",
                ...
                success: function (data) {
                    ...
                    $("#dangerousEdit").val(dataJson.dangerousType);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-chemical").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var dangerousType=$("#dangerousEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-chemical").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#chemical-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/dangerous/chemicals/facility/export?name="+name;
            $.ajax({
                url: srcPath + "admin/dangerous/chemicals/facility/export?name="+name,
                type: "get",
