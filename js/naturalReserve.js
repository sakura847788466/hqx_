...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/natural/reserve/list",
            type:"post",
            dataType:'json',
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].level == "" || dataJson[i].level == null || dataJson[i].level == undefined) {
                            ...
                            dataJson[i].levellevel = dataJson[i].level;
                        }
                        if (dataJson[i].protectedObject == "" || dataJson[i].protectedObject == null || dataJson[i].protectedObject == undefined) {
                            dataJson[i].protectedObject = "无";
                        } else {
                            dataJson[i].protectedObject = dataJson[i].protectedObject;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].protectedObject + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    naturalReserveIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                naturalReserveIds.splice(0,naturalReserveIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var naturalReserveId=$(this).siblings("td").eq(0).text();
                    naturalReserveIds.push(naturalReserveId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(naturalReserveIds.indexOf(txt) == "-1"){

                    }else{
                        naturalReserveIds.splice(naturalReserveIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#naturalReserve-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#naturalReserve-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
            ...
            $(".filtrate-item").click(function () {
                ...
                if($(this).hasClass("filtrate-click")){
                    ...
                    var tab=$("#naturalReserve-table")[0];
                    var trs=tab.rows;
    ...
    //新建
    $("#naturalReserve-add").click(function () {
    ...
    $(".add-close").click(function () {
        ...
        $("#naturalReserve-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".naturalReserve-tips").hide();
        $("#add-pop .imgList-naturalReserve").hide();
        $("#edit-pop .imgList-naturalReserve").hide();
        $("#add-pop .imgList-naturalReserve").html("");
        $("#edit-pop .imgList-naturalReserve").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".naturalReserve-tips").show();
            $(".naturalReserve-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".naturalReserve-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".naturalReserve-tips").show();
            $(".naturalReserve-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".naturalReserve-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-naturalReserve").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                    ...
                    $("#add-pop .imgList-naturalReserve").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        var name=$("#name").val();
        var level=$("#level").val();
        var protectedObject=$("#object").val();
        var area=$("#area").val();
        ...
        if($("#add-pop").find("div.imgList-naturalReserve").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/natural/reserve/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".naturalReserve-tips").show();
            $(".naturalReserve-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".naturalReserve-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#naturalReserve-delAll").click(function () {
        $("#delAll-pop").show();
        if(naturalReserveIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#naturalReserve-delAll").removeClass("table-menuClick");
        $("#naturalReserve-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/natural/reserve/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/natural/reserve/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.protectedObject==""||dataJson.protectedObject==null||dataJson.protectedObject==undefined){
                        dataJson.protectedObject="无";
                    }else{
                        dataJson.protectedObject=dataJson.protectedObject;
                    }
                    ...
                    $("#objectInfo").text(dataJson.protectedObject);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-naturalReserve").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-naturalReserve").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            naturalReserveId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/natural/reserve/detail",
                ...
                success: function (data) {
                    ...
                    $("#objectEdit").val(dataJson.protectedObject);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-naturalReserve").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var protectedObject=$("#objectEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-naturalReserve").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#naturalReserve-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/natural/reserve/export?name="+name;
            $.ajax({
                url: srcPath + "admin/natural/reserve/export?name="+name,
                type: "get",
