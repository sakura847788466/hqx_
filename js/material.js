...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/material/reserve/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].storageCapacity == "" || dataJson[i].storageCapacity == null || dataJson[i].storageCapacity == undefined) {
                            dataJson[i].storageCapacity = "无";
                        } else {
                            dataJson[i].storageCapacity = dataJson[i].storageCapacity;
                        }
                        if (dataJson[i].totalCapacity == "" || dataJson[i].totalCapacity == null || dataJson[i].totalCapacity == undefined) {
                            dataJson[i].totalCapacity = "无";
                        } else {
                            dataJson[i].totalCapacity = dataJson[i].totalCapacity;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].storageCapacity + "</td>\n" +
                            "            <td>" + dataJson[i].totalCapacity + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    materialIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                materialIds.splice(0,materialIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var materialId=$(this).siblings("td").eq(0).text();
                    materialIds.push(materialId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(materialIds.indexOf(txt) == "-1"){
    
                    }else{
                        materialIds.splice(materialIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#material-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#material-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".material-tips").hide();
        $("#add-pop .imgList-material").hide();
        $("#edit-pop .imgList-material").hide();
        $("#add-pop .imgList-material").html("");
        $("#edit-pop .imgList-material").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".material-tips").show();
            $(".material-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".material-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".material-tips").show();
            $(".material-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".material-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-material").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        var name=$("#name").val();
        var type=$("#type").val();
        var storageCapacity=$("#storage").val();
        ...
        if($("#add-pop").find("div.imgList-material").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/material/reserve/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".material-tips").show();
            $(".material-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".material-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#material-delAll").click(function () {
        $("#delAll-pop").show();
        if(materialIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#material-delAll").removeClass("table-menuClick");
        $("#material-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/material/reserve/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/material/reserve/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.storageCapacity==""||dataJson.storageCapacity==null||dataJson.storageCapacity==undefined){
                        dataJson.storageCapacity="无";
                    }else{
                        dataJson.storageCapacity=dataJson.storageCapacity;
                    }
                    if(dataJson.totalCapacity==""||dataJson.totalCapacity==null||dataJson.totalCapacity==undefined){
                        dataJson.totalCapacity="无";
                    }else{
                        dataJson.totalCapacity=dataJson.totalCapacity;
                    }
                    ...
                    $("#storageInfo").text(dataJson.storageCapacity);
                    $("#totalCapacityInfo").text(dataJson.totalCapacity);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-material").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-material").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            materialId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/material/reserve/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.storageCapacity==""||dataJson.storageCapacity==null||dataJson.storageCapacity==undefined){
                        dataJson.storageCapacity="";
                    }else{
                        dataJson.storageCapacity=dataJson.storageCapacity;
                    }
                    if(dataJson.totalCapacity==""||dataJson.totalCapacity==null||dataJson.totalCapacity==undefined){
                        dataJson.totalCapacity="";
                    }else{
                        dataJson.totalCapacity=dataJson.totalCapacity;
                    }
                    if(dataJson.totalInvestment==""||dataJson.totalInvestment==null||dataJson.totalInvestment==undefined){
                    ...
                    $("#storageEdit").val(dataJson.storageCapacity);
                    $("#totalCapacityEdit").val(dataJson.totalCapacity);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-material").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var storageCapacity=$("#storageEdit").val();
                var totalCapacity=$("#totalCapacityEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-material").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#material-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/material/reserve/export?name="+name;
            $.ajax({
                url: srcPath + "admin/material/reserve/export?name="+name,
                type: "get",
