...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/substation/facility/list",
            type:"post",
            dataType:'json',
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    powerIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                powerIds.splice(0,powerIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var powerId=$(this).siblings("td").eq(0).text();
                    powerIds.push(powerId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(powerIds.indexOf(txt) == "-1"){

                    }else{
                        powerIds.splice(powerIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#power-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#power-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".power-tips").hide();
        $("#add-pop .imgList-power").hide();
        $("#edit-pop .imgList-power").hide();
        $("#add-pop .imgList-power").html("");
        $("#edit-pop .imgList-power").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".power-tips").show();
            $(".power-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".power-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".power-tips").show();
            $(".power-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".power-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-power").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        if($("#add-pop").find("div.imgList-power").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/substation/facility/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".power-tips").show();
            $(".power-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".power-tips").hide();
            },1000);
        }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#power-delAll").removeClass("table-menuClick");
        $("#power-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/substation/facility/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/substation/facility/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-power").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            powerId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/substation/facility/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-power").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                if($("#edit-pop").find("div.imgList-power").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#power-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/substation/facility/export?name="+name;
            $.ajax({
                url: srcPath + "admin/substation/facility/export?name="+name,
                type: "get",
