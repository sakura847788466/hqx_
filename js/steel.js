...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/steel/base/list",
            type:"post",
            dataType:'json',
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    steelIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                steelIds.splice(0,steelIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var steelId=$(this).siblings("td").eq(0).text();
                    steelIds.push(steelId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(steelIds.indexOf(txt) == "-1"){
    
                    }else{
                        steelIds.splice(steelIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#steel-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".steel-tips").hide();
        $("#add-pop .imgList-steel").hide();
        $("#edit-pop .imgList-steel").hide();
        $("#add-pop .imgList-steel").html("");
        $("#edit-pop .imgList-steel").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".steel-tips").show();
            $(".steel-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".steel-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".steel-tips").show();
            $(".steel-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".steel-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-steel").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        if($("#add-pop").find("div.imgList-steel").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/steel/base/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".steel-tips").show();
            $(".steel-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".steel-tips").hide();
            },1000);
        }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#steel-delAll").removeClass("table-menuClick");
        $("#steel-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/steel/base/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/steel/base/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-steel").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            steelId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/steel/base/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-steel").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                if($("#edit-pop").find("div.imgList-steel").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#steel-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/steel/base/export?name="+name;
            $.ajax({
                url: srcPath + "admin/steel/base/export?name="+name,
                type: "get",
