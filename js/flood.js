...
$(function () {
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for(var i=0;i<dataJson.length;i++){
                        ...
                        if(dataJson[i].designStandard=="" || dataJson[i].designStandard==null || dataJson[i].designStandard==undefined){
                            dataJson[i].designStandard="无";
                        }else {
                            dataJson[i].designStandard=dataJson[i].designStandard;
                        }
                        if(dataJson[i].sluiceDischarge=="" || dataJson[i].sluiceDischarge==null || dataJson[i].sluiceDischarge==undefined){
                            dataJson[i].sluiceDischarge="无";
                        }else {
                            dataJson[i].sluiceDischarge=dataJson[i].sluiceDischarge;
                        }
                        if(dataJson[i].sluiceCreateTime=="" || dataJson[i].sluiceCreateTime==null || dataJson[i].sluiceCreateTime==undefined){
                            dataJson[i].sluiceCreateTime="无";
                        }else {
                            dataJson[i].sluiceCreateTime=new Date(dataJson[i].sluiceCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML=tableHTML+"<tr>\n" +
                            ...
                            "            <td>"+dataJson[i].designStandard+"</td>\n" +
                            "            <td>"+dataJson[i].sluiceDischarge+"</td>\n" +
                            "            <td>"+dataJson[i].sluiceCreateTime+"</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    floodIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                floodIds.splice(0,floodIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var floodId=$(this).siblings("td").eq(0).text();
                    floodIds.push(floodId);
                } else {
                    var txt=$(this).siblings("td").eq(0).text();
                    $(".table").find("thead tr th").removeClass("check-click");
                    if(floodIds.indexOf(txt) == "-1"){
    
                    }else{
                        floodIds.splice(floodIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#flood-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $("#edit-pop .imgList-flood").hide();
        $("#edit-pop .imgList-flood").html("");
        $("#add-pop .imgList-flood").hide();
        $("#add-pop .imgList-flood").html("");
        $(".flood-tips").hide();
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".flood-tips").show();
            $(".flood-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".flood-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".flood-tips").show();
            $(".flood-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".flood-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-flood").append(imgList);
                    uploadImgDelAdd();
    ...
    var sluiceCreateTimeAdd,fillCreateTimeAdd,checkTimeAdd;
    layui.use('laydate', function(){
        ...
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                sluiceCreateTimeAdd=Date.parse(value);
            }
        });
        laydate.render({
    ...
    $("#addSure").click(function () {
        var name=$("#name").val();
        var type=$("#type").val();
        var designStandard=$("#standard").val();
        var sluiceDischarge=$("#discharge").val();
        //var sluiceCreateTime=$("#createTime").val();
        ...
        if($("#add-pop").find("div.imgList-flood").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax(){
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    "sluiceDischarge": sluiceDischarge,
                    "sluiceCreateTime": sluiceCreateTimeAdd,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
                    "areaCode": areaCode,
        ...
        if(name==""){
            $(".flood-tips").show();
            $(".flood-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".flood-tips").hide();
            },1000);
        }else if(designStandard!=""){
            if(!(/^(-?\d+)(\.\d+)?$/.test(designStandard))){
                $(".flood-tips").show();
                $(".flood-tips").find("span").text("设计标准必须为数字类型");
                setTimeout(function () {
                    $(".flood-tips").hide();
                },1000);
            }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#flood-delAll").removeClass("table-menuClick");
        $("#flood-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/sluice/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/sluice/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.designStandard==""||dataJson.designStandard==null||dataJson.designStandard==undefined){
                        dataJson.designStandard="无";
                    }else{
                        dataJson.designStandard=dataJson.designStandard;
                    }
                    if(dataJson.sluiceDischarge==""||dataJson.sluiceDischarge==null||dataJson.sluiceDischarge==undefined){
                        dataJson.sluiceDischarge="无";
                    }else{
                        dataJson.sluiceDischarge=dataJson.sluiceDischarge;
                    }
                    if(dataJson.sluiceCreateTime==""||dataJson.sluiceCreateTime==null||dataJson.sluiceCreateTime==undefined){
                        dataJson.sluiceCreateTime="无";
                    }else{
                        dataJson.sluiceCreateTime=new Date(dataJson.sluiceCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#standardInfo").text(dataJson.designStandard);
                    $("#dischargeInfo").text(dataJson.sluiceDischarge);
                    $("#createTimeInfo").text(dataJson.sluiceCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-flood").append(imgList);
    ...
    var sluiceCreateTimeEdit,fillCreateTimeEdit,checkTimeEdit;
    layui.use('laydate', function(){
        var laydate = layui.laydate;
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                sluiceCreateTimeEdit=Date.parse(value);
            }
        });
        laydate.render({
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            floodId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/sluice/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.designStandard==""||dataJson.designStandard==null||dataJson.designStandard==undefined){
                        dataJson.designStandard="";
                    }else{
                        dataJson.designStandard=dataJson.designStandard;
                    }
                    if(dataJson.sluiceDischarge==""||dataJson.sluiceDischarge==null||dataJson.sluiceDischarge==undefined){
                        dataJson.sluiceDischarge="";
                    }else{
                        dataJson.sluiceDischarge=dataJson.sluiceDischarge;
                    }
                    if(dataJson.sluiceCreateTime==""||dataJson.sluiceCreateTime==null||dataJson.sluiceCreateTime==undefined){
                        dataJson.sluiceCreateTime="";
                    }else{
                        dataJson.sluiceCreateTime=new Date(dataJson.sluiceCreateTime).Format("yyyy-MM-dd");
                    }
                    if(dataJson.address==""||dataJson.address==null||dataJson.address==undefined){
                    ...
                    $("#standardEdit").val(dataJson.designStandard);
                    $("#dischargeEdit").val(dataJson.sluiceDischarge);
                    $("#createTimeEdit").val(dataJson.sluiceCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        var imgList="";
                        for(var i=0;i<dataJson.picArray.length;i++){
                            // var imgSrc="/image"+(dataJson.picArray[i]).split("image")[1];
                            // uploadImgListOld.push(imgSrc);
                            imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                                "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
            ...
            $("#editSure").click(function () {
                ...
                var designStandard=$("#standardEdit").val();
                var sluiceDischarge=$("#dischargeEdit").val();
                //var sluiceCreateTime=$("#createTimeEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-flood").css("display")=="none"){
                    uploadImgLists.splice(0,uploadImgLists.length)
                }else{
                ...
                function ajax(){
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "designStandard": designStandard,
                            "sluiceDischarge": sluiceDischarge,
                            "sluiceCreateTime": sluiceCreateTimeEdit,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
                            "areaCode": areaCode,
    ...
    //导出
    $("#flood-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/sluice/export?name="+name;
            $.ajax({
                url: srcPath + "admin/sluice/export?name="+name,
                type: "get",
