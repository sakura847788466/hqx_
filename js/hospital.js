...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/hospital/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (String(dataJson[i].bedNumber) == "" || dataJson[i].bedNumber == null || dataJson[i].bedNumber == undefined) {
                            dataJson[i].bedNumber = "无";
                        } else {
                            dataJson[i].bedNumber = dataJson[i].bedNumber;
                        }
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    hospitalIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                hospitalIds.splice(0,hospitalIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var hospitalId=$(this).siblings("td").eq(0).text();
                    hospitalIds.push(hospitalId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(hospitalIds.indexOf(txt) == "-1"){

                    }else{
                        hospitalIds.splice(hospitalIds.indexOf(txt),1);
                    }
                }
            });
        })
    ...
    function tableFilter() {
        var tableList=[];
        $("#hospital-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#hospital-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".hospital-tips").hide();
        $("#add-pop .imgList-hospital").hide();
        $("#edit-pop .imgList-hospital").hide();
        $("#add-pop .imgList-hospital").html("");
        $("#edit-pop .imgList-hospital").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".hospital-tips").show();
            $(".hospital-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".hospital-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".hospital-tips").show();
            $(".hospital-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".hospital-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-hospital").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        if($("#add-pop").find("div.imgList-hospital").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/hospital/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".hospital-tips").show();
            $(".hospital-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".hospital-tips").hide();
            },1000);
        }else if(staffNumber!=""){
            if(!(/^(-?\d+)(\.\d+)?$/.test(staffNumber))){
                $(".hospital-tips").show();
                $(".hospital-tips").find("span").text("现有职工必须为数字类型");
                setTimeout(function () {
                    $(".hospital-tips").hide();
                },1000);
            }else{
                //ajax();
                if(bedNumber!=""){
                    if(!(/^(-?\d+)(\.\d+)?$/.test(bedNumber))){
                        $(".hospital-tips").show();
                        $(".hospital-tips").find("span").text("核定床位数必须为数字类型");
                        setTimeout(function () {
                            $(".hospital-tips").hide();
                        },1000);
                    }else{
    ...
    //批量删除/
    $("#hospital-delAll").click(function () {
        $("#delAll-pop").show();
        if(hospitalIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#hospital-delAll").removeClass("table-menuClick");
        $("#hospital-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/hospital/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/hospital/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(String(dataJson.bedNumber)==""||dataJson.bedNumber==null||dataJson.bedNumber==undefined){
                        dataJson.bedNumber="无";
                    }else{
                        dataJson.bedNumber=dataJson.bedNumber;
                    }
                    ...
                    $("#bedInfo").text(dataJson.bedNumber);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-hospital").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-hospital").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            hospitalId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/hospital/detail",
                ...
                success: function (data) {
                    ...
                    $("#bedEdit").val(dataJson.bedNumber);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-hospital").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                if($("#edit-pop").find("div.imgList-hospital").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#hospital-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/hospital/export?name="+name;
            $.ajax({
                url: srcPath + "admin/hospital/export?name="+name,
                type: "get",
