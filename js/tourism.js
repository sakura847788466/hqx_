...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/tourist/entertainment/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (String(dataJson[i].dailyVisitorNumber) == "" || dataJson[i].dailyVisitorNumber == null || dataJson[i].dailyVisitorNumber == undefined) {
                            dataJson[i].dailyVisitorNumber = "无";
                        } else {
                            dataJson[i].dailyVisitorNumber = dataJson[i].dailyVisitorNumber;
                        }
                        if (String(dataJson[i].peakSeasonDailyVisitorNumber) == "" || dataJson[i].peakSeasonDailyVisitorNumber == null || dataJson[i].peakSeasonDailyVisitorNumber == undefined) {
                            dataJson[i].peakSeasonDailyVisitorNumber = "无";
                        } else {
                            dataJson[i].peakSeasonDailyVisitorNumber = dataJson[i].peakSeasonDailyVisitorNumber;
                        }
                        ...
                        if (dataJson[i].zoneCreateTime == "" || dataJson[i].zoneCreateTime == null || dataJson[i].zoneCreateTime == undefined) {
                            dataJson[i].zoneCreateTime = "无";
                        } else {
                            dataJson[i].zoneCreateTime = new Date(dataJson[i].zoneCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].dailyVisitorNumber + "</td>\n" +
                            "            <td>" + dataJson[i].peakSeasonDailyVisitorNumber + "</td>\n" +
                            "            <td>" + dataJson[i].area + "</td>\n" +
                            "            <td>" + dataJson[i].zoneCreateTime + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    tourismIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                tourismIds.splice(0,tourismIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var tourismId=$(this).siblings("td").eq(0).text();
                    tourismIds.push(tourismId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(tourismIds.indexOf(txt) == "-1"){

                    }else{
                        tourismIds.splice(tourismIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#tourism-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#tourism-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#tourism-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".tourism-tips").hide();
        $("#add-pop .imgList-tourism").hide();
        $("#edit-pop .imgList-tourism").hide();
        $("#add-pop .imgList-tourism").html("");
        $("#edit-pop .imgList-tourism").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".tourism-tips").show();
            $(".tourism-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".tourism-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".tourism-tips").show();
            $(".tourism-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".tourism-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-tourism").append(imgList);
                    uploadImgDelAdd();
    ...
    var facilityCreateTimeAdd,fillCreateTimeAdd,checkTimeAdd;
    layui.use('laydate', function(){
        ...
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                facilityCreateTimeAdd=Date.parse(value);
            }
        });
        laydate.render({
    ...
    $("#addSure").click(function () {
        ...
        var type=$("#type").val();
        var level=$("#level").val();
        var dailyVisitorNumber=$("#dailyNum").val();
        var peakSeasonDailyVisitorNumber=$("#seasonNum").val();
        var area=$("#area").val();
        //var parkCreateTime=$("#createTime").val();
        //var createTime=new Date(parkCreateTime).getTime();
        ...
        if($("#add-pop").find("div.imgList-tourism").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/tourist/entertainment/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "type": type,
                    "level":level,
                    "dailyVisitorNumber":dailyVisitorNumber,
                    "peakSeasonDailyVisitorNumber":peakSeasonDailyVisitorNumber,
                    "area":area,
                    "zoneCreateTime":facilityCreateTimeAdd,
                    "altitude":altitude,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
        ...
        if(name==""){
            $(".tourism-tips").show();
            $(".tourism-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".tourism-tips").hide();
            },1000);
        }else if(dailyVisitorNumber!=""){
            if(!(/^(-?\d+)(\.\d+)?$/.test(dailyVisitorNumber))){
                $(".tourism-tips").show();
                $(".tourism-tips").find("span").text("设计日游客接待量必须为数字类型");
                setTimeout(function () {
                    $(".tourism-tips").hide();
                },1000);
            }else{
                //ajax();
                if(peakSeasonDailyVisitorNumber!="") {
                    if (!(/^(-?\d+)(\.\d+)?$/.test(peakSeasonDailyVisitorNumber))) {
                        $(".tourism-tips").show();
                        $(".tourism-tips").find("span").text("旺季日游客接待量必须为数字类型");
                        setTimeout(function () {
                            $(".tourism-tips").hide();
                        },1000);
                    } else {
        ...
        }else if(peakSeasonDailyVisitorNumber!="") {
            if (!(/^(-?\d+)(\.\d+)?$/.test(peakSeasonDailyVisitorNumber))) {
                $(".tourism-tips").show();
                $(".tourism-tips").find("span").text("旺季日游客接待量必须为数字类型");
                setTimeout(function () {
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#tourism-delAll").removeClass("table-menuClick");
        $("#tourism-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/tourist/entertainment/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/tourist/entertainment/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(String(dataJson.dailyVisitorNumber)==""||dataJson.dailyVisitorNumber==null||dataJson.dailyVisitorNumber==undefined){
                        dataJson.dailyVisitorNumber="无";
                    }else{
                        dataJson.dailyVisitorNumber=dataJson.dailyVisitorNumber;
                    }
                    if(String(dataJson.peakSeasonDailyVisitorNumber)==""||dataJson.peakSeasonDailyVisitorNumber==null||dataJson.peakSeasonDailyVisitorNumber==undefined){
                        dataJson.peakSeasonDailyVisitorNumber="无";
                    }else{
                        dataJson.peakSeasonDailyVisitorNumber=dataJson.peakSeasonDailyVisitorNumber;
                    }
                    ...
                    if(dataJson.zoneCreateTime==""||dataJson.zoneCreateTime==null||dataJson.zoneCreateTime==undefined){
                        dataJson.zoneCreateTime="无";
                    }else{
                        dataJson.zoneCreateTime=new Date(dataJson.zoneCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#dailyNumInfo").text(dataJson.dailyVisitorNumber);
                    $("#seasonNumInfo").text(dataJson.peakSeasonDailyVisitorNumber);
                    $("#areaInfo").text(dataJson.area);
                    $("#createTimeInfo").text(dataJson.zoneCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-tourism").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    var facilityCreateTimeEdit,fillCreateTimeEdit,checkTimeEdit;
    layui.use('laydate', function(){
        var laydate = layui.laydate;
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                facilityCreateTimeEdit=Date.parse(value);
            }
        });
        laydate.render({
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            tourismId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/tourist/entertainment/detail",
                ...
                success: function (data) {
                    ...
                    $("#dailyNumEdit").val(dataJson.dailyVisitorNumber);
                    $("#seasonNumEdit").val(dataJson.peakSeasonDailyVisitorNumber);
                    $("#areaEdit").val(dataJson.area);
                    $("#createTimeEdit").val(dataJson.zoneCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-tourism").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var dailyVisitorNumber=$("#dailyNumEdit").val();
                var peakSeasonDailyVisitorNumber=$("#seasonNumEdit").val();
                var area=$("#areaEdit").val();
                //var parkCreateTime=$("#createTimeEdit").val();
                //var createTime=new Date(parkCreateTime).getTime();
                ...
                if($("#edit-pop").find("div.imgList-tourism").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
                ...
                function ajax() {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "peakSeasonDailyVisitorNumber":peakSeasonDailyVisitorNumber,
                            "area":area,
                            "zoneCreateTime":facilityCreateTimeEdit,
                            "altitude":altitude,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
    ...
    //导出
    $("#tourism-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/tourist/entertainment/export?name="+name;
            $.ajax({
                url: srcPath + "admin/tourist/entertainment/export?name="+name,
                type: "get",
