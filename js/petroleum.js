...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/petrochemical/industry/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].productCategory == "" || dataJson[i].productCategory == null || dataJson[i].productCategory == undefined) {
                            dataJson[i].productCategory = "无";
                        } else {
                            dataJson[i].productCategory = dataJson[i].productCategory;
                        }
                        if (dataJson[i].annualOutput == "" || dataJson[i].annualOutput == null || dataJson[i].annualOutput == undefined) {
                            dataJson[i].annualOutput = "无";
                        } else {
                            dataJson[i].annualOutput = dataJson[i].annualOutput;
                        }
                        ...
                        if (dataJson[i].facilityCreateTime == "" || dataJson[i].facilityCreateTime == null || dataJson[i].facilityCreateTime == undefined) {
                            dataJson[i].facilityCreateTime = "无";
                        } else {
                            dataJson[i].facilityCreateTime = new Date(dataJson[i].facilityCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].productCategory + "</td>\n" +
                            "            <td>" + dataJson[i].annualOutput + "</td>\n" +
                            "            <td>" + dataJson[i].totalInvestment + "</td>\n" +
                            "            <td>" + dataJson[i].area + "</td>\n" +
                            "            <td>" + dataJson[i].facilityCreateTime + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    petroleumIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                petroleumIds.splice(0,petroleumIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var petroleumId=$(this).siblings("td").eq(0).text();
                    petroleumIds.push(petroleumId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(petroleumIds.indexOf(txt) == "-1"){
    
                    }else{
                        petroleumIds.splice(petroleumIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#petroleum-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#petroleum-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#petroleum-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".petroleum-tips").hide();
        $("#add-pop .imgList-petroleum").hide();
        $("#edit-pop .imgList-petroleum").hide();
        $("#add-pop .imgList-petroleum").html("");
        $("#edit-pop .imgList-petroleum").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".petroleum-tips").show();
            $(".petroleum-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".petroleum-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".petroleum-tips").show();
            $(".petroleum-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".petroleum-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-petroleum").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                    ...
                    $("#add-pop .imgList-petroleum").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        var annualOutput=$("#output").val();
        var totalInvestment=$("#total").val();
        var area=$("#area").val();
        //var facilityCreateTime=$("#createTime").val();
        //var createTime=new Date(facilityCreateTime).getTime();
        ...
        if($("#add-pop").find("div.imgList-petroleum").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/petrochemical/industry/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "area":area,
                    "facilityCreateTime":facilityCreateTimeAdd,
                    "altitude":altitude,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
        ...
        if(name==""){
            $(".petroleum-tips").show();
            $(".petroleum-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".petroleum-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#petroleum-delAll").click(function () {
        $("#delAll-pop").show();
        if(petroleumIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#petroleum-delAll").removeClass("table-menuClick");
        $("#petroleum-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/petrochemical/industry/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/petrochemical/industry/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.productCategory==""||dataJson.productCategory==null||dataJson.productCategory==undefined){
                        dataJson.productCategory="无";
                    }else{
                        dataJson.productCategory=dataJson.productCategory;
                    }
                    if(dataJson.annualOutput==""||dataJson.annualOutput==null||dataJson.annualOutput==undefined){
                        dataJson.annualOutput="无";
                    }else{
                        dataJson.annualOutput=dataJson.annualOutput;
                    }
                    ...
                    if(dataJson.facilityCreateTime==""||dataJson.facilityCreateTime==null||dataJson.facilityCreateTime==undefined){
                        dataJson.facilityCreateTime="无";
                    }else{
                        dataJson.facilityCreateTime=new Date(dataJson.facilityCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#categoryInfo").text(dataJson.productCategory);
                    $("#outputInfo").text(dataJson.annualOutput);
                    $("#totalInfo").text(dataJson.totalInvestment);
                    $("#areaInfo").text(dataJson.area);
                    $("#createTimeInfo").text(dataJson.facilityCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-petroleum").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-petroleum").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            petroleumId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/petrochemical/industry/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.productCategory==""||dataJson.productCategory==null||dataJson.productCategory==undefined){
                        dataJson.productCategory="";
                    }else{
                        dataJson.productCategory=dataJson.productCategory;
                    }
                    if(dataJson.annualOutput==""||dataJson.annualOutput==null||dataJson.annualOutput==undefined){
                        dataJson.annualOutput="";
                    }else{
                        dataJson.annualOutput=dataJson.annualOutput;
                    }
                    if(dataJson.totalInvestment==""||dataJson.totalInvestment==null||dataJson.totalInvestment==undefined){
                    ...
                    $("#categoryEdit").val(dataJson.productCategory);
                    $("#outputEdit").val(dataJson.annualOutput);
                    $("#totalEdit").val(dataJson.totalInvestment);
                    $("#areaEdit").val(dataJson.area);
                    $("#createTimeEdit").val(dataJson.facilityCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-petroleum").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var productCategory=$("#categoryEdit").val();
                var annualOutput=$("#outputEdit").val();
                var totalInvestment=$("#totalEdit").val();
                var area=$("#areaEdit").val();
                //var facilityCreateTime=$("#createTimeEdit").val();
                //var createTime=new Date(facilityCreateTime).getTime();
                ...
                if($("#edit-pop").find("div.imgList-petroleum").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
                ...
                function ajax() {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "totalInvestment":totalInvestment,
                            "area":area,
                            "facilityCreateTime":facilityCreateTimeEdit,
                            "altitude":altitude,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
    ...
    //导出
    $("#petroleum-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/petrochemical/industry/export?name="+name;
            $.ajax({
                url: srcPath + "admin/petrochemical/industry/export?name="+name,
                type: "get",
