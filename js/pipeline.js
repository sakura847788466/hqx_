...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/oil/gas/transportation/list",
            type:"post",
            dataType:'json',
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    pipelineIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                pipelineIds.splice(0,pipelineIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var pipelineId=$(this).siblings("td").eq(0).text();
                    pipelineIds.push(pipelineId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(pipelineIds.indexOf(txt) == "-1"){

                    }else{
                        pipelineIds.splice(pipelineIds.indexOf(txt),1);
                    }
                }
            });
        })
    ...
    function tableFilter() {
        var tableList=[];
        $("#pipeline-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#pipeline-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#pipeline-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".pipeline-tips").hide();
        $("#add-pop .imgList-pipeline").hide();
        $("#edit-pop .imgList-pipeline").hide();
        $("#add-pop .imgList-pipeline").html("");
        $("#edit-pop .imgList-pipeline").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".pipeline-tips").show();
            $(".pipeline-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".pipeline-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".pipeline-tips").show();
            $(".pipeline-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".pipeline-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-pipeline").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
       ...
       //var checkTime=$("#checkTime").val();
        ...
        if($("#add-pop").find("div.imgList-pipeline").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/oil/gas/transportation/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        if(name==""){
            $(".pipeline-tips").show();
            $(".pipeline-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".pipeline-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#pipeline-delAll").click(function () {
        $("#delAll-pop").show();
        if(pipelineIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#pipeline-delAll").removeClass("table-menuClick");
        $("#pipeline-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/oil/gas/transportation/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/oil/gas/transportation/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-pipeline").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-pipeline").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            pipelineId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/oil/gas/transportation/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-pipeline").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                if($("#edit-pop").find("div.imgList-pipeline").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#pipeline-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/oil/gas/transportation/export?name="+name;
            $.ajax({
                url: srcPath + "admin/oil/gas/transportation/export?name="+name,
                type: "get",
