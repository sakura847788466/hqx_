...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/shipyard/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (String(dataJson[i].annualOutput) == "" || dataJson[i].annualOutput == null || dataJson[i].annualOutput == undefined) {
                            dataJson[i].annualOutput = "无";
                        } else {
                            dataJson[i].annualOutput = dataJson[i].annualOutput;
                        }
                        if (String(dataJson[i].totalInvestment) == "" || dataJson[i].totalInvestment == null || dataJson[i].totalInvestment == undefined) {
                        ...
                        if (dataJson[i].shipyardCreateTime == "" || dataJson[i].shipyardCreateTime == null || dataJson[i].shipyardCreateTime == undefined) {
                            dataJson[i].shipyardCreateTime = "无";
                        } else {
                            dataJson[i].shipyardCreateTime = new Date(dataJson[i].shipyardCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].shipyardCreateTime + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    shipIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                shipIds.splice(0,shipIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var shipId=$(this).siblings("td").eq(0).text();
                    shipIds.push(shipId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(shipIds.indexOf(txt) == "-1"){
    
                    }else{
                        shipIds.splice(shipIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#ship-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#ship-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".ship-tips").hide();
        $("#add-pop .imgList-ship").hide();
        $("#edit-pop .imgList-ship").hide();
        $("#add-pop .imgList-ship").html("");
        $("#edit-pop .imgList-ship").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".ship-tips").show();
            $(".ship-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".ship-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".ship-tips").show();
            $(".ship-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".ship-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-ship").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        var area=$("#area").val();
        //var shipyardCreateTime=$("#createTime").val();
       //var createTime=new Date(shipyardCreateTime).getTime();
        ...
        if($("#add-pop").find("div.imgList-ship").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/shipyard/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "area":area,
                    "shipyardCreateTime":facilityCreateTimeAdd,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
                    "areaCode": areaCode,
        ...
        if(name==""){
            $(".ship-tips").show();
            $(".ship-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".ship-tips").hide();
            },1000);
        }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#ship-delAll").removeClass("table-menuClick");
        $("#ship-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/shipyard/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/shipyard/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(String(dataJson.annualOutput)==""||dataJson.annualOutput==null||dataJson.annualOutput==undefined){
                        dataJson.annualOutput="无";
                    }else{
                        dataJson.annualOutput=dataJson.annualOutput;
                    }
                    if(String(dataJson.totalInvestment)==""||dataJson.totalInvestment==null||dataJson.totalInvestment==undefined){
                    ...
                    if(dataJson.shipyardCreateTime==""||dataJson.shipyardCreateTime==null||dataJson.shipyardCreateTime==undefined){
                        dataJson.shipyardCreateTime="无";
                    }else{
                        dataJson.shipyardCreateTime=new Date(dataJson.shipyardCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#createTimeInfo").text(dataJson.shipyardCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-ship").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            shipId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/shipyard/detail",
                ...
                success: function (data) {
                    ...
                    $("#createTimeEdit").val(dataJson.shipyardCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-ship").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                //var shipyardCreateTime=$("#createTimeEdit").val();
                //var createTime=new Date(shipyardCreateTime).getTime();
                ...
                if($("#edit-pop").find("div.imgList-ship").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
                ...
                function ajax() {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "totalInvestment":totalInvestment,
                            "area":area,
                            "shipyardCreateTime":facilityCreateTimeEdit,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
                            "areaCode": areaCode,
    ...
    //导出
    $("#ship-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/shipyard/export?name="+name;
            $.ajax({
                url: srcPath + "admin/shipyard/export?name="+name,
                type: "get",
