...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/generating/facility/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].characteristic == "" || dataJson[i].characteristic == null || dataJson[i].characteristic == undefined) {
                            dataJson[i].characteristic = "无";
                        } else {
                            dataJson[i].characteristic = dataJson[i].characteristic;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].characteristic + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    electricityIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                electricityIds.splice(0,electricityIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var electricityId=$(this).siblings("td").eq(0).text();
                    electricityIds.push(electricityId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(electricityIds.indexOf(txt) == "-1"){

                    }else{
                        electricityIds.splice(electricityIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#electricity-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#electricity-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    //新建
    $("#electricity-add").click(function () {
    ...
    $(".add-close").click(function () {
        ...
        $("#electricity-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".electricity-tips").hide();
        $("#add-pop .imgList-electricity").hide();
        $("#add-pop .imgList-electricity").html("");
        $("#edit-pop .imgList-electricity").hide();
        $("#edit-pop .imgList-electricity").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".electricity-tips").show();
            $(".electricity-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".electricity-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".electricity-tips").show();
            $(".electricity-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".electricity-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-electricity").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                    ...
                    $("#add-pop .imgList-electricity").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        var name=$("#name").val();
        var type=$("#type").val();
        var characteristic=$("#character").val();
        ...
        if($("#add-pop").find("div.imgList-electricity").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/generating/facility/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "area":area,
                    "facilityCreateTime":fillCreateTimeAdd,
                    "altitude":altitude,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
        ...
        if(name==""){
            $(".haven-tips").show();
            $(".electricity-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".electricity-tips").hide();
            },1000);
            $(".electricity-tips").text("名称不能为空");
        }else{
            ajax();
        }
    });
    //批量删除
    $("#electricity-delAll").click(function () {
        $("#delAll-pop").show();
        if(electricityIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#electricity-delAll").removeClass("table-menuClick");
        $("#electricity-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/generating/facility/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/generating/facility/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.characteristic==""||dataJson.characteristic==null||dataJson.characteristic==undefined){
                        dataJson.characteristic="无";
                    }else{
                        dataJson.characteristic=dataJson.characteristic;
                    }
                    ...
                    $("#characterInfo").text(dataJson.characteristic);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-electricity").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-electricity").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            electricityId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/generating/facility/detail",
                ...
                success: function (data) {
                    ...
                    $("#characterEdit").val(dataJson.characteristic);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-electricity").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var characteristic=$("#characterEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-electricity").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#electricity-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/generating/facility/export?name="+name;
            $.ajax({
                url: srcPath + "admin/generating/facility/export?name="+name,
                type: "get",
