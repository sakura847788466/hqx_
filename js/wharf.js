...
$(function () {
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].subordinate == "" || dataJson[i].subordinate == null || dataJson[i].subordinate == undefined) {
                            dataJson[i].subordinate = "无";
                        } else {
                            dataJson[i].subordinate = dataJson[i].subordinate;
                        }
                        ...
                        if (String(dataJson[i].berthsNumber) == "" || dataJson[i].berthsNumber == null || dataJson[i].berthsNumber == undefined) {
                            dataJson[i].berthsNumber = "无";
                        } else {
                            dataJson[i].berthsNumber = dataJson[i].berthsNumber;
                        }
                        if (String(dataJson[i].shipPassNumber) == "" || dataJson[i].shipPassNumber == null || dataJson[i].shipPassNumber == undefined) {
                            dataJson[i].shipPassNumber = "无";
                        } else {
                            dataJson[i].shipPassNumber = dataJson[i].shipPassNumber;
                        }
                        if (dataJson[i].builtScale == "" || dataJson[i].builtScale == null || dataJson[i].builtScale == undefined) {
                            dataJson[i].builtScale = "无";
                        } else {
                            dataJson[i].builtScale = dataJson[i].builtScale;
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].subordinate + "</td>\n" +
                            "            <td>" + dataJson[i].type + "</td>\n" +
                            "            <td>" + dataJson[i].berthsNumber + "</td>\n" +
                            "            <td>" + dataJson[i].shipPassNumber + "</td>\n" +
                            "            <td>" + dataJson[i].builtScale + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    wharfIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                wharfIds.splice(0,wharfIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var wharfId=$(this).siblings("td").eq(0).text();
                    wharfIds.push(wharfId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(wharfIds.indexOf(txt) == "-1"){
    
                    }else{
                        wharfIds.splice(wharfIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#wharf-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $("#edit-pop .imgList-wharf").hide();
        $("#add-pop .imgList-wharf").hide();
        $("#edit-pop .imgList-wharf").html("");
        $("#add-pop .imgList-wharf").html("");
        $(".wharf-tips").hide();
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".wharf-tips").show();
            $(".wharf-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".wharf-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".wharf-tips").show();
            $(".wharf-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".wharf-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-wharf").append(imgList);
                    uploadImgDelAdd();
    ...
    $("#addSure").click(function () {
        ...
        var berthsNumber=$("#berthNum").val();
        var shipPassNumber=$("#shipPass").val();
        var builtScale=$("#built").val();
        ...
        if($("#add-pop").find("div.imgList-wharf").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    "berthsNumber":berthsNumber,
                    "shipPassNumber":shipPassNumber,
        ...
        if(name==""){
            $(".wharf-tips").show();
            $(".wharf-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".wharf-tips").hide();
            },1000);
        }else if(berthsNumber!=""){
            if(!(/^(-?\d+)(\.\d+)?$/.test(berthsNumber))){
                $(".wharf-tips").show();
                $(".wharf-tips").find("span").text("泊位个数必须为数字类型");
                setTimeout(function () {
                    $(".wharf-tips").hide();
                },1000);
            }else{
                //ajax();
                if(shipPassNumber!=""){
                    if(!(/^(-?\d+)(\.\d+)?$/.test(shipPassNumber))){
                        $(".wharf-tips").show();
                        $(".wharf-tips").find("span").text("船舶通航量必须为数字类型");
                        setTimeout(function () {
                            $(".wharf-tips").hide();
                        },1000);
                    }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#wharf-delAll").removeClass("table-menuClick");
        $("#wharf-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/wharf/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/wharf/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.subordinate==""||dataJson.subordinate==null||dataJson.subordinate==undefined){
                        dataJson.subordinate="无";
                    }else{
                        dataJson.subordinate=dataJson.subordinate;
                    }
                    if(String(dataJson.berthsNumber)==""||dataJson.berthsNumber==null||dataJson.berthsNumber==undefined){
                        dataJson.berthsNumber="无";
                    }else{
                        dataJson.berthsNumber=dataJson.berthsNumber;
                    }
                    if(String(dataJson.shipPassNumber)==""||dataJson.shipPassNumber==null||dataJson.shipPassNumber==undefined){
                        dataJson.shipPassNumber="无";
                    }else{
                        dataJson.shipPassNumber=dataJson.shipPassNumber;
                    }
                    if(dataJson.builtScale==""||dataJson.builtScale==null||dataJson.builtScale==undefined){
                        dataJson.builtScale="无";
                    }else{
                        dataJson.builtScale=dataJson.builtScale;
                    }
                    ...
                    $("#subordinateInfo").text(dataJson.subordinate);
                    $("#berthNumInfo").text(dataJson.berthsNumber);
                    $("#shipPassInfo").text(dataJson.shipPassNumber);
                    $("#builtInfo").text(dataJson.builtScale);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-wharf").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            wharfId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/wharf/detail",
                ...
                success: function (data) {
                    ...
                    if(dataJson.subordinate==""||dataJson.subordinate==null||dataJson.subordinate==undefined){
                        dataJson.subordinate="";
                    }else{
                        dataJson.subordinate=dataJson.subordinate;
                    }
                    if(String(dataJson.berthsNumber)==""||dataJson.berthsNumber==null||dataJson.berthsNumber==undefined){
                        dataJson.berthsNumber="";
                    }else{
                        dataJson.berthsNumber=dataJson.berthsNumber;
                    }
                    if(String(dataJson.shipPassNumber)==""||dataJson.shipPassNumber==null||dataJson.shipPassNumber==undefined){
                        dataJson.shipPassNumber="";
                    }else{
                        dataJson.shipPassNumber=dataJson.shipPassNumber;
                    }
                    if(dataJson.builtScale==""||dataJson.builtScale==null||dataJson.builtScale==undefined){
                    ...
                    $("#subordinateEdit").val(dataJson.subordinate);
                    $("#berthNumEdit").val(dataJson.berthsNumber);
                    $("#shipPassEdit").val(dataJson.shipPassNumber);
                    $("#builtEdit").val(dataJson.builtScale);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-wharf").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var subordinate=$("#subordinateEdit").val();
                var berthsNumber=$("#berthNumEdit").val();
                var shipPassNumber=$("#shipPassEdit").val();
                var builtScale=$("#builtEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-wharf").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
    ...
    //导出
    $("#wharf-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/wharf/export?name="+name;
            $.ajax({
                url: srcPath + "admin/wharf/export?name="+name,
                type: "get",
