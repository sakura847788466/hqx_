...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/railway/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].railwayCreateTime == "" || dataJson[i].railwayCreateTime == null || dataJson[i].railwayCreateTime == undefined) {
                            dataJson[i].railwayCreateTime = "无";
                        } else {
                            dataJson[i].railwayCreateTime = new Date(dataJson[i].railwayCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].railwayCreateTime + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    railwayIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                railwayIds.splice(0,railwayIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var railwayId=$(this).siblings("td").eq(0).text();
                    railwayIds.push(railwayId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(railwayIds.indexOf(txt) == "-1"){
    
                    }else{
                        railwayIds.splice(railwayIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#railway-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
        tableList.splice(0,2);
        tableList.splice(tableList.length-3,tableList.length);
        $("#railway-select").click(function () {
            $(".filtrate-pop").toggle();
            var html="";
    ...
    $(".add-close").click(function () {
        ...
        $("#railway-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".railway-tips").hide();
        $("#edit-pop .imgList-railway").hide();
        $("#add-pop .imgList-railway").hide();
        $("#edit-pop .imgList-railway").html("");
        $("#add-pop .imgList-railway").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".railway-tips").show();
            $(".railway-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".railway-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".railway-tips").show();
            $(".railway-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".railway-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-railway").append(imgList);
                    uploadImgDelAdd();
    ...
    var railwayCreateTimeAdd,fillCreateTimeAdd,checkTimeAdd;
    layui.use('laydate', function(){
        ...
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                railwayCreateTimeAdd=Date.parse(value);
            }
        });
        laydate.render({
    ...
    $("#addSure").click(function () {
        ...
        var speed=$("#speed").val();
        //var railwayCreateTime=$("#createTime").val();
        //var createTime=new Date(railwayCreateTime).getTime();
        ...
        if($("#add-pop").find("div.imgList-railway").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/railway/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "speed":speed,
                    "railwayCreateTime":railwayCreateTimeAdd,
                    "altitude":altitude,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
        ...
        if(name==""){
            $(".railway-tips").show();
            $(".railway-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".railway-tips").hide();
            },1000);
        }else{
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#railway-delAll").removeClass("table-menuClick");
        $("#railway-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/railway/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/railway/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.railwayCreateTime==""||dataJson.railwayCreateTime==null||dataJson.railwayCreateTime==undefined){
                        dataJson.railwayCreateTime="无";
                    }else{
                        dataJson.railwayCreateTime=new Date(dataJson.railwayCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#createTimeInfo").text(dataJson.railwayCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#edit-pop .imgList-railway").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    var railwayCreateTimeEdit,fillCreateTimeEdit,checkTimeEdit;
    layui.use('laydate', function(){
        var laydate = layui.laydate;
        laydate.render({
            ...
            change:function(value,data){//监听日期变换
                ...
                railwayCreateTimeEdit=Date.parse(value);
            }
        });
        laydate.render({
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            railwayId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/railway/detail",
                ...
                success: function (data) {
                    ...
                    $("#createTimeEdit").val(dataJson.railwayCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-railway").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                //var railwayCreateTime=$("#createTimeEdit").val();
                //var createTime=new Date(railwayCreateTime).getTime();
                ...
                if($("#edit-pop").find("div.imgList-railway").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
                ...
                function ajax() {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "dailyCarPassNumber":dailyCarPassNumber,
                            "speed":speed,
                            "railwayCreateTime":railwayCreateTimeEdit,
                            "altitude":altitude,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
    ...
    //导出
    $("#railway-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/railway/export?name="+name;
            $.ajax({
                url: srcPath + "admin/railway/export?name="+name,
                type: "get",
