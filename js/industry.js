...
$(function () {
    ...
    function page(page,size,searchName){
        ...
        $.ajax({
            url:srcPath+"admin/industrial/park/list",
            type:"post",
            dataType:'json',
            ...
            success:function(data){
                ...
                if(dataJson==""){
                    ...
                    for (var i = 0; i < dataJson.length; i++) {
                        ...
                        if (dataJson[i].industrialOutput == "" || dataJson[i].industrialOutput == null || dataJson[i].industrialOutput == undefined) {
                        ...
                        if (dataJson[i].parkCreateTime == "" || dataJson[i].parkCreateTime == null || dataJson[i].parkCreateTime == undefined) {
                            dataJson[i].parkCreateTime = "无";
                        } else {
                            dataJson[i].parkCreateTime = new Date(dataJson[i].parkCreateTime).Format('yyyy-MM-dd');
                        }
                        ...
                        tableHTML = tableHTML + "<tr>\n" +
                            ...
                            "            <td>" + dataJson[i].parkCreateTime + "</td>\n" +
    ...
    function tableCheck(){
        $(".table").find("thead tr th:eq(0)").click(function () {
            ...
            if($(this).hasClass("check-click")){
                ...
                for(var i=0;i<tableLength.length;i++){
                    industryIds.push(tableLength[i].id);
                }
            }else {
                $(".table").find("tbody tr td").removeClass("check-click");
                industryIds.splice(0,industryIds.length);
            }
        });
        $(".table").find("tbody tr").each(function(){
            ...
            td.click(function(){
                $(this).toggleClass("check-click");
                if ($(this).hasClass("check-click")){
                    var industryId=$(this).siblings("td").eq(0).text();
                    industryIds.push(industryId);
                } else {
                    $(".table").find("thead tr th").removeClass("check-click");
                    var txt=$(this).siblings("td").eq(0).text();
                    if(industryIds.indexOf(txt) == "-1"){
    
                    }else{
                        industryIds.splice(industryIds.indexOf(txt),1);
                    }
                }
            });
        });
    ...
    function tableFilter() {
        var tableList=[];
        $("#industry-table").find("thead tr th").each(function () {
            var txt = $(this).text();
            tableList.push(txt);
        });
    ...
    $(".add-close").click(function () {
        ...
        $("#industry-add").removeClass("table-menuClick");
        $(".overlay",window.parent.document).hide();
        $(".add-input").val("");
        $(".industry-tips").hide();
        $("#add-pop .imgList-industry").hide();
        $("#edit-pop .imgList-industry").hide();
        $("#add-pop .imgList-industry").html("");
        $("#edit-pop .imgList-industry").html("");
    });
    ...
    function upload(ele){
        ...
        if (uploadImgList.length + files.length > 6) {
            $(".industry-tips").show();
            $(".industry-tips").find("span").text("最多只能上传6张图片，请重新上传");
            setTimeout(function () {
                $(".industry-tips").hide();
            },1000);
            return;
        ...
        if (errorText) {
            $(".industry-tips").show();
            $(".industry-tips").find("span").text(errorText);
            setTimeout(function () {
                $(".industry-tips").hide();
            },1000);
            return;
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    $("#add-pop .imgList-industry").append(imgList);
                    uploadImgDelAdd();
            ...
            error : function (responseStr) {
                $("#uploadForm").siblings("span").text("上传失败");
                function upload_default(){
                    $("#uploadForm").siblings("span").text("点击上传");
                }
                setTimeout(upload_default,1000);
            }
        });
    }
    //图片删除和放大效果
    ...
    $("#addSure").click(function () {
        ...
        var type=$("#type").val();
        var level=$("#level").val();
        var industrialOutput=$("#output").val();
        var area=$("#area").val();
        ...
        if($("#add-pop").find("div.imgList-industry").css("display")==="none"){
            uploadImgList.splice(0,uploadImgList.length);
        }else{
        ...
        function ajax() {
            $.ajax({
                url: srcPath + "admin/industrial/park/save",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                data: JSON.stringify({
                    ...
                    "area":area,
                    "parkCreateTime":facilityCreateTimeAdd,
                    "address": address,
                    "longitude": longitude,
                    "latitude": latitude,
                    "areaCode": areaCode,
        ...
        if(name==""){
            $(".industry-tips").show();
            $(".industry-tips").find("span").text("名称不能为空");
            setTimeout(function () {
                $(".industry-tips").hide();
            },1000);
        }else{
    ...
    //批量删除
    $("#industry-delAll").click(function () {
        $("#delAll-pop").show();
        if(industryIds==""){
    ...
    $(".del-btnIcon").click(function () {
        ...
        $("#industry-delAll").removeClass("table-menuClick");
        $("#industry-export").removeClass("table-menuClick");
    });
    ...
    $("#delAllSure").click(function () {
        $.ajax({
            url: srcPath + "admin/industrial/park/delete",
            type: "post",
            dataType: 'json',
            contentType: 'application/json',
            headers: {
    ...
    function tableInfo() {
        $(".table-infoIcon").click(function () {
            ...
            $("#info-pop").show();
            $.ajax({
                url: srcPath + "admin/industrial/park/detail",
                type: "post",
                dataType: 'json',
                contentType: 'application/json',
                headers: {
                ...
                success: function (data) {
                    ...
                    if(dataJson.industrialOutput==""||dataJson.industrialOutput==null||dataJson.industrialOutput==undefined){
                    ...
                    if(dataJson.parkCreateTime==""||dataJson.parkCreateTime==null||dataJson.parkCreateTime==undefined){
                        dataJson.parkCreateTime="无";
                    }else{
                        dataJson.parkCreateTime=new Date(dataJson.parkCreateTime).Format("yyyy-MM-dd");
                    }
                    ...
                    $("#outputInfo").text(dataJson.industrialOutput);
                    $("#areaInfo").text(dataJson.area);
                    $("#createTimeInfo").text(dataJson.parkCreateTime);
        ...
        $.ajax({
            ...
            success: function (responseStr) {
                if(responseStr.code=="-2"){
                    ...
                    var dataJson=responseStr.data.list;
                    $("#edit-pop .imgList-industry").show();
                    var imgList="";
                    for(var i=0;i<dataJson.length;i++){
                        imgList=imgList+"<div class=\"seawallNumber-imgItem\">\n" +
                            "                                    <i class=\"iconfont upload-imgIcon\">&#xeac5;</i>\n" +
                    ...
                    $("#edit-pop .imgList-industry").append(imgList);
                    uploadImgDelEdit();
                }else if(responseStr.code=="-1"){
                    $("#fail").show().find("span").text(responseStr.msg);
                    setTimeout(function(){
    ...
    function tableEdit() {
        $(".table-editIcon").click(function () {
            ...
            industryId=$(this).parent().siblings("td").eq(1).text();
            $("#edit-pop").show();
            $(".overlay",window.parent.document).show();
            $.ajax({
                url: srcPath + "admin/industrial/park/detail",
                ...
                success: function (data) {
                    ...
                    $("#outputEdit").val(dataJson.industrialOutput);
                    $("#areaEdit").val(dataJson.area);
                    $("#createTimeEdit").val(dataJson.parkCreateTime);
                    ...
                    if(dataJson.picArray==""||dataJson.picArray==null||dataJson.picArray==undefined){
                        ...
                        $("#edit-pop .imgList-industry").show().append(imgList);
                        uploadImgDelEdit();
                    }
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            $("#editSure").click(function () {
                ...
                var industrialOutput=$("#outputEdit").val();
                ...
                if($("#edit-pop").find("div.imgList-industry").css("display")==="none"){
                    uploadImgLists.splice(0,uploadImgLists.length);
                }else{
                ...
                function ajax() {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            "industrialOutput":industrialOutput,
                            "area":area,
                            "parkCreateTime":facilityCreateTimeEdit,
                            "address": address,
                            "longitude": longitude,
                            "latitude": latitude,
                            "areaCode": areaCode,
    ...
    //导出
    $("#industry-export").click(function () {
        var name=$("#searchName").val();
        $("#export-pop").show();
        $("#exportSure").click(function () {
            $("#loading").show();
            $(".overlayOpacity",window.parent.document).show();
            var url=srcPath + "admin/industrial/park/export?name="+name;
            $.ajax({
                url: srcPath + "admin/industrial/park/export?name="+name,
                type: "get",
